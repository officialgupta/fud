  ...
  task send: :environment do
    ...
    User.all.each do |user|
      if user.items.select{|i| i.expired_in?(7)}.count > 0
        ...
        message = api.messages.build( :to => user.phone, :content => "Your #{items} will expire in three days, consider using them. Reply to 84477 with item:status for each item to update it's status to in-use, used, donated or disposed." )
